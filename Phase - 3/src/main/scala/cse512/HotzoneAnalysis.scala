...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val jointDf = spark.sql("select rectangle, count(point) as pointcount from joinResult group by rectangle order by rectangle asc").persist()
    jointDf.createOrReplaceTempView("jointResult2")
    //jointDf.show()
    
    //val jointddDf = spark.sql("select rectangle, pointcount from jointResult");
    //jointddDf.createOrReplaceTempView("jointResult")
    //jointddDf.show()
    //print(jointddDf.count())
    
//    val jointDf2 = spark.sql("select rectangle, pointcount from jointResult where pointcount != 10000 order by pointcount asc")
//    jointDf2.createOrReplaceTempView("joint2Result")
//    jointDf2.show()
    // YOU NEED TO CHANGE THIS PART

    return jointDf // YOU NEED TO CHANGE THIS PART
  }
